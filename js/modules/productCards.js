const rendersProductCard = ({ id, photo, model, prices }) => {
  const li = document.createElement('li')
  li.classList.add('product', 'item', 'column', 'aic', 'js-product')
  li.innerHTML = `<div class="favorites js-favorites">
					...
					<a
						href="#"
						id="${id}"
						class="link column aic js-link-card"
					>
						<div class="product-image row jcc">
							<img
							src="img/products/${photo[0]}"
							alt=""
							class="image js-image-card"
							/>
						</div>
					...
					<button type="button" class="addCart buy-button js-buy-button">
						В корзину
					</button>`
  return li
}

const rendersProductCards = (products, container) => {
  products.forEach((product) => {
    const card = rendersProductCard(product)
    container.append(card)
  })
}

export { rendersProductCards }
